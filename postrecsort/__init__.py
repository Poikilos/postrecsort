...
def usage():
    echo0("{} <photorec result directory with recup_dir.*> <profile>"
          "".format(sys.argv[0]))


def paddedMessage(msg):
    usage()
    echo0("")
    if msg is not None:
        echo0("ERROR:")
...
ignoreExts = [
    "ani", "api", "asp", "ax", "bat", "cnv", "cp_", "cpl",
    "class", "dat", "db", "dll", "cab", "chm", "edb", "elf", "emf", "exe", "f",
    "h", "icc", "ime", "ini", "jar", "java", "js", "jsp", "lib", "loc",
    "mui", "ocx", "olb", "reg", "rll", "sam", "scr", "sports", "sqm", "swc",
    "sys", "sys_place_holder_for_2k_and_xp_(see_pxhelp)",
...
foundMaximumPaths = {}
extDesc = {}
extDesc['mmw'] = ("AceMoney Money File, MechCAD Software LLC financial file,"
                  " or misnamed dangerous executable")
    ...
    for subName in sortedBySize(folderPath, os.listdir(folderPath)):
        ...
        if not os.path.isfile(subPath):
            if subName not in doneNames:
                removeExtra(subPath, profilePath, relPath=subRelPath,
                            depth=depth+1)
            continue
        ...
        if category == "Pictures":
            ...
            try:
                ...
                isBlank = True
                clearCount = 0
                print('# checking pixels in {0:.2g} MB "{}"'
                      ''.format(fileSize/1024/1024, subPath))
                ...
                if not isBlank:
                    ...
                    if prevIm is not None:
                        ...
                        if prevIm.size == im.size:
                            isDup = True
                            # print('# comparing pixels in "{}"...'
                            #       ''.format(subPath))
                            for y in range(height):
                                for x in range(width):
                                    if (rgbIm.getpixel((x, y))
                                            != prevIm.getpixel((x, y))):
                                        isDup = False
                                        break
                        prevIm.close()
            ...
            if (prevSize is not None) and (fileSize == prevSize) \
                ...
                print('# comparing bytes to {0:.2g} MB "{}"'
                      ''.format(fileSize/1024/1024, prevDestPath))
...
def sortFiles(preRecoveredPath, profilePath, relPath="", depth=0,
    ...
    global catDirNames
    if not os.path.isdir(preRecoveredPath):
        paddedMessage(preRecoveredPath + " is not a directory")
        return 1
    ...
    for subName in os.listdir(folderPath):
        ...
        if os.path.isfile(subPath):
            ...
            if category is None:
                ...
                catMajorPath = os.path.join(profilePath, catDirNames[category])
            if category == "Music":
                # echo0('This track is by %s.' % tag.artist)
                # echo0("  " * depth + subPath + ": " + str(tag))
                ...
                # if artist == "unknown":
                #     debug only arst
                #     echo0("unknown artist in " + str(newStats))
                #     return 4
                if newName is None:
                    newName = subName
            ...
            if os.path.isfile(newPath):
                if fileSize <= os.path.getsize(newPath):
                    ...
                    print("# removing smaller '" + newPath
                          + "' and keeping '" + subPath + "'")
                    os.remove(newPath)

            # while os.path.isfile(newPath):
                # tryNum += 1
                # newPath = os.path.join(catPath, newNamePartial
                #                        + " [" + str(tryNum) + "]")
            ...
            if enableShowLarge:
                if fileSize > largeSize:
                    echo0('{0:.3g}'.format(fileSize/1024/1024)
                          + "MB: " + newPath)
                    # g removes insignificant zeros
            if ((category not in foundMaximums)
                    or (fileSize > foundMaximums[category])):
            ...
            if lowerExt not in foundTypeCounts.keys():
                ...
                foundTypeCounts[lowerExt] += 1
            if category is None:
                echo0("  " * depth + "unknown type: " + lowerExt
                      + " for '" + subPath + "'")
            # echo0("  " * depth + "[" + str(category) + "]" + newPath)
        elif os.path.isdir(subPath):
            # echo0(" " * depth + subName)
            # if subName[:1] != ".":
            code = sortFiles(subPath, profilePath, depth=depth+1)
            if code != 0:
                return code
    return 0
...
def main():
    if len(sys.argv) < 3:
        paddedMessage("There must be two parameters.")
        return 3
    code = sortFiles(sys.argv[1], sys.argv[2])
    if code != 0:
        # An error should already have been shown.
        return code
    enableCleanup = True
    for argI in range(len(sys.argv)):
        ...
        if argI == 0:
            ...
            if arg == "--nocleanup":
                ...
                paddedMessage("Unknown option: {}".format(arg))
                return 2
    if enableCleanup:
    ...
    for k, v in foundMaximums.items():
        print("  Largest in " + k + ":" + '{0:.3g}'.format(v/1024/1024)
              + " MB): " + foundMaximumPaths[k])
